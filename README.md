...
<div align="center">
<img width="4715" height="4853" alt="SOSHexArchDiagram" src="https://github.com/user-attachments/assets/5e926392-b48a-40e6-adc9-2cb8bc899897" />
</div>  
<br/>
<p>
...
La arquitectura Hexagonal tiene el concepto de adaptadores primarios (clases de servicios de la aplicación cercanos al usuario), los cuales dependen de implementaciones de los puertos secundarios (contratos de los adaptadores secundarios) para obtener información de servicios externos, en este caso, el gestor de base de datos SQL Server. De esta forma, la parte Modelo del patrón arquitectónico MVC que originalmente fue pensado para el sistema, tenga una separación considerable de responsabilidades.
</p>
<p>
...
<div align="center"><img width="1472" height="2465" alt="SOSArchDependencies" src="https://github.com/user-attachments/assets/61bd5630-7dda-4221-8228-af2b1a0b877a" /></div>
<p>
La integración de esta arquitectura al sistema de ordenes de servicio se realiza con el patrón de diseño "factory", creando 2 clases destinadas para los index.php de home y finishing que devuelven un objeto de la clase ContainerBuilder de PHP-DI, una dependencia que permite usar el patrón de diseño "dependency injection", la mayoría de las clases de la arquitectura y los controladores del sistema de ordenes de servicio siguen el requisito de este patrón de diseño en sus constructores. El objeto de la clase ContainerBuilder, normalmente llamado $container, tiene la definición de todas las dependencias necesarias (dependiendo de cada index.php) y se usa el método make para llamar cada dependencia teniendo un comportamiento tipo transient (objetos "frescos") en cada solicitud http.
</p>
<div align="center"><img width="848" height="145" alt="images" src="https://github.com/user-attachments/assets/e5c59ab8-016d-4216-8e80-641d1d43ece1" /></div>
<p>
Tanto el código frontend JavaScript como el backend PHP del sistema de ordenes de servicio se les añadió una suite de pruebas Jest y Pest respectivamente, en Pest hay tanto pruebas de característica como unitarias de los controladores y helpers, en Jest se hacen mocks del funcionamiento del canvas del pad de firmas para las pruebas unitarias de esta funcionalidad. De manera adicional, en los test de la carpeta features de Pest se añadió documentación de los controladores y los helpers del sistema, de igual manera, en la parte de Jest, dentro de los archivos de las pruebas se colocó documentación de la funcionalidad del lado del cliente del sistema.
</p>
