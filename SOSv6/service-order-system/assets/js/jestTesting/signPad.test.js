/*
    ...
    El archivo SignPad.js alberga la logica del "pad" de las firmas, la logica se divide en 4 partes,
    la primer parte consiste en esta función tipo "callback", esto es que se utiliza o llama en una parte en concreto de ese archivo JS y el de finishing.js, generateCanvas
    ...
    medio del objeto de formulario de javascript que se inicializó al principio del arhivo finishing.js para que PHP lo interprete como un archivo enviado por un formulario 
    ...
    const dataURLtoBlob = (dataURL) => {
        ...
        se accede al indice 1 del indice 0 de parts separado por ':'. 
        ...
        //Despues de haber guardado cada caracter del string binario a cada indice de Uint8Array, se retorna un objeto de la clase Blob el cual necesita un array object 
...
*/
const signPad = require('../UIFunctions/signPad.js');
const { generateCanvas, dataURLtoBlob, isCanvasBlank } = require('../UIFunctions/signPad.js');
const {fireEvent} = require('@testing-library/dom');
require('jest-canvas-mock');

describe('pruebas de generateSignPad, callback del archivo finishing.js', ()=> {
    ...
    beforeEach(() =>{
        ...
        Object.defineProperty(window, 'innerHeight', {
         ...
         document.body.innerHTML = `
            <main class="binnacle-signmain">
            <canvas class="binnacle-sign__canvas" id="technicianCanvas"></canvas>
            <label class="binnacle-sign__label">Firma del tecnico</label>
            <div class="binnacle-sign__window-background hidThis" id="backWindow">
            <div class="binnacle-sign__info-window" id="infoWindow"></div>
            </div>

            <div class="binnacle-sign__buttons-box" id="buttonsBox">
                <button class="buttons-box__button" id="cancelButton">cancelar</button>
                <button class="buttons-box__button" id="cleanButton">limpiar</button>
                <button class="buttons-box__button" id="nextButton">Seguir</button>
            </div>
            </main>
        `;
        
        backWindow = document.querySelector("#backWindow");
        infoWindow = document.querySelector("#infoWindow");
        buttonsBox = document.querySelector("#buttonsBox");
        cancelButton = document.querySelector("#cancelButton");
        cleanButton = document.querySelector("#cleanButton");
        nextButton = document.querySelector("#nextButton");

        BASE_URL = 'http://localhost:8081/SOSv5/service-order-system/';
        technicianCanvas = document.querySelector("#technicianCanvas");
        clientCanvas = document.querySelector("#clientCanvas");
        formData = new FormData();
        serverData = {
            binnId: 14,
            userId: 2,
            userName: 'Laura Lorena',
            userSurname: 'Morales Lop0z'
        };

        isCanvasPinted = false;
        ...
        Object.defineProperty(technicianCanvas, 'offsetLeft', { value: 20, configurable: true });
        Object.defineProperty(technicianCanvas, 'offsetTop', { value: 30, configurable: true });
        technicianCanvas.getContext = jest.fn().mockReturnValue(mockCtx);
        context = technicianCanvas.getContext('2d');

        context.getImageData = jest.fn((x, y, width, height) => {
            const totalBytes = width * height * 4; // 4 bytes por píxel (RGBA)
            const arregloClamped = new Uint8ClampedArray(totalBytes);

            if (isCanvasPinted) 
                arregloClamped[3] = 255; 

            return {
                width,
                height,
                // Al usar un objeto real Uint8ClampedArray, la propiedad .buffer existirá nativamente
                data: arregloClamped 
            };
        });

        spy = jest.spyOn(signPad, 'generateCanvas').mockImplementation(()=>{
            isCanvasPinted = false;        
        });

        mockBase64 = "data:image/png;base64,"
    });
    ...
    test('caso click en botón "cancelar"', ()=>{
        //lógica parcial de la función generateSignPad:
        ...
        buttonsBox.addEventListener("click", e => {
            ...
            if (e.target.id === "cleanButton") {
                ...
                cancelTextBoxInner = textBox.innerHTML;
                cancelbuttonsBoxInner = buttonsBox.innerHTML;        
        ...
        expect(cancelTextBoxInner).toBe('<h3>¿quieres regresar a la parte de conformidad de actividades?</h3>');
        expect(cancelbuttonsBoxInner).toBe('<a class="selectbuttons-box__button ok" id="ok" href="http://localhost:8081/SOSv5/service-order-system/finishing/?controller=followupform&amp;action=index&amp;id=14">Si</a><button class="selectbuttons-box__button" id="no">No</button>');

    });

    test('Caso click en botón "limpiar" y "seguir"', ()=>{
        //lógica parcial de la función generateSignPad:
        ...
        buttonsBox.addEventListener("click", e => {
            ...
            if (e.target.id === "cleanButton") {
                ...
                (technicianCanvas != null) ? 
                signPad.generateCanvas(technicianCanvas, window.innerWidth, 
                window.innerHeight) :
                generateCanvas(clientCanvas, window.innerWidth, 
                window.innerHeight);
                ...
                const buttonsArea = infoWindow.querySelector(".info-window__selectbuttons-box");
                const nextYesBtn = infoWindow.querySelector("#yes");
                    ...
                    if (e.target.id === "yes") {
                        ...
                        if(technicianCanvas != null){
                            if(isCanvasBlank(technicianCanvas)){
                                ...
                                cleanTextBoxInner = textBox.innerHTML;
        ...
        fireEvent.mouseDown(technicianCanvas);
        fireEvent.mouseMove(technicianCanvas, { clientX: 120, clientY: 150 });

        expect(context.lineTo).toHaveBeenCalledWith(100, 150);
        expect(context.stroke).toHaveBeenCalled();
        expect(isCanvasPinted).toBe(true);

        cleanButton.click();

        expect(spy).toHaveBeenCalled();
        expect(isCanvasPinted).toBe(false);

        nextButton.click();

        expect(cleanTextBoxInner).toBe('<h3>Debes de pintar tu firma antes de seguir</h3>');
    });

    test('Caso technicianCanvas en la firma de una bitácora', ()=>{
        //lógica parcial de la función generateSignPad:
            ...
            if (e.target.id === "cleanButton") {
                ...
                buttonsArea.addEventListener("click", e => {
                    ...
                    if (e.target.id === "yes") {
                        ...
                        if(technicianCanvas != null){
                            if(!isCanvasBlank(technicianCanvas)){
                                technicianCanvas.toDataURL = jest.fn().mockReturnValue(mockBase64);
                                const dataURL = technicianCanvas.toDataURL('image/png');
                                const imageBlob = dataURLtoBlob(dataURL);
                                formData.append((serverData.binnId != null) ? "techSign" : "newTechSign", imageBlob, `userid_${serverData.userId}_${serverData.userName.split(" ").join("_")}_${serverData.userSurname.split(" ").join("_")}_Sign.png`);
                            }
                        ...
                        if(formData.has("techSign") || formData.has("cliSign") || formData.has("newTechSign")){
                            ...
                            */    
                                buttonsBox.classList.add("okCenter");
                                iconBox.innerHTML = `<img class="pop-up-window-icon__img" src="${BASE_URL}assets/img/check_13526142.png"/>`;
                                textBox.innerHTML = `<h3>La firma del técnico se creó con éxito</h3>`;
                                ...
                                techButtonsBoxInner = buttonsBox.innerHTML;    
        ...
        expect(formData.get('techSign')).toBeDefined();
        expect(formData.get('techSign')).toBeInstanceOf(Blob);
        expect(formData.get('techSign').name).toBe('userid_2_Laura_Lorena_Morales_Lop0z_Sign.png');
        
        expect(techButtonsBoxInner).toBe('<a class="selectbuttons-box__button ok" id="ok" href="http://localhost:8081/SOSv5/service-order-system/finishing/?controller=followupform&amp;action=clientSign">OK</a>');
        
    });

    test('Caso clientCanvas en la firma de una bitácora', ()=>{
        document.body.innerHTML = `
            <main class="binnacle-signmain">
            <canvas class="binnacle-sign__canvas" id="clientCanvas"></canvas>
            <label class="binnacle-sign__label">Firma del cliente</label>
            <div class="binnacle-sign__window-background hidThis" id="backWindow">
            <div class="binnacle-sign__info-window" id="infoWindow"></div>
            </div>
        ...
        const BASE_URL = 'http://localhost:8081/SOSv5/service-order-system/';
        ...
        const serverData = {
            binnId: 14,
            clientName: 'Elena Aurora Rodriguez',
            altClientName: 'Metalurgica Salvatierra'
        };

        Object.defineProperty(clientCanvas, 'offsetLeft', { value: 20, configurable: true });
        Object.defineProperty(clientCanvas, 'offsetTop', { value: 30, configurable: true });
        clientCanvas.getContext = jest.fn().mockReturnValue(mockCtx);
                    ...
                    if (e.target.id === "yes") {
                        ...
                        if(formData.has("techSign") || formData.has("cliSign") || formData.has("newTechSign")){
                            ...
                            */    
                                ...
                                cliButtonsBoxInner = buttonsBox.innerHTML;    
        ...
        fireEvent.mouseDown(clientCanvas);
        fireEvent.mouseMove(clientCanvas, { clientX: 120, clientY: 150 });
        nextButton.click();
        ...
        expect(formData.get('cliSign')).toBeDefined();
        expect(formData.get('cliSign')).toBeInstanceOf(Blob);
        expect(formData.get('cliSign').name).toBe('bitacoraid_14_Elena_Aurora_Rodriguez_Metalurgica_Salvatierra_Sign.png');
        
        expect(cliButtonsBoxInner).toBe('<a class="selectbuttons-box__button ok" id="ok"' +
                                    ' href="'+BASE_URL+'finishing/?controller=followupform&amp;action=finishbinnacle">OK</a>');
        
    });

    test('Caso technicianCanvas en la edición de una firma de usuario', ()=>{
        const serverData = {
            userId: 2,
            userName: 'Laura Lorena',
            userSurname: 'Morales Lop0z'
                    ...
                    if (e.target.id === "yes") {
                        ...
                        if(formData.has("techSign") || formData.has("cliSign") || formData.has("newTechSign")){
                            ...
                            */    
                                ...
                                editButtonsBoxInner = buttonsBox.innerHTML;    
        ...
        expect(formData.get('newTechSign')).toBeDefined();
        expect(formData.get('newTechSign')).toBeInstanceOf(Blob);
        expect(formData.get('newTechSign').name).toBe('userid_2_Laura_Lorena_Morales_Lop0z_Sign.png');
        
        expect(editButtonsBoxInner).toBe('<a class="selectbuttons-box__button ok" id="ok"' +
                                    ' href="'+BASE_URL+'home/?homeController=user&amp;homeAction=editSign">OK</a>');
